  ...
  return (
    <TouchableOpacity onPress={onPress} className='flex flex-col items-start w-60 h-60 relative'>
      ...
      <View className='flex flex-col items-start absolute bottom-5 inset-x-5'>
        <Text className='text-xl font-rubik-extrabold text-white ' numberOfLines={1}>{item.type}</Text>
...
export const Card = ({item,onPress}:props) => {
    ...
    return (
      ...
      <View className='flex flex-col mt-2'>
        <Text className='text-base font-rubik-bold text-black-300 '>{item.type}</Text>
